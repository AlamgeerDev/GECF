...
{
    public class DefinitionsPageViewModel : BaseViewModel
    {

        INavigation navigation;
        public DefinitionsPageViewModel(INavigation navigation)
        {
        ...
        public async Task DoAction()
        {
            ...
            var apicall = await GECFAPI.Instance.GetDefinitionsNewsAsync();
            foreach (var item in apicall)
            {
                if (item.source != null)
                {
                    item.source = "Source: " + item.source ?? "NA";
                }
                else
                {
                    item.source = "Source: " + "NA";
                }
            }
            var group = apicall.OrderBy(e => e.title)
                 .GroupBy(e => e.title.ToString())
                 .Select(e => new ObsevablecollectionforGrouping<string, DefinitionList>(e))
                 .ToList();
            if (group.Count != 0)
            {
                CatNewsList = group;
        ...
        private List<ObsevablecollectionforGrouping<string, DefinitionList>> _catNewsList;
        public List<ObsevablecollectionforGrouping<string, DefinitionList>> CatNewsList
        {
            get
            {
                return _catNewsList;
